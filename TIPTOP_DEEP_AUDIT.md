...
| Vidéo | Demandée dans le prompt (« TikTok/Reels ») | **Décision documentée** : TipTop n'a pas de pipeline vidéo (upload, transcodage, lecteur) dans cette base de code — seules des images (`/seed/*.svg`) sont supportées. Plutôt que de simuler une fonctionnalité vidéo inexistante (interdit par la règle anti-fake-UI, #69), l'immersion verticale a été implémentée sur le média réellement supporté (photo/texte plein cadre). Le composant `MoodSlide` est structuré pour accueillir un futur `<video>` sans réécriture si un pipeline média est ajouté. |
