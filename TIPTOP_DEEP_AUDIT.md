...
| Adresse / carte sur une vidéo Mood | PARTIEL — seuls `city`/`zone` existaient, toujours hérités du profil, sans adresse précise ni itinéraire | → **Ajouté** : lieu **optionnel** (`placeName`, `address`, `latitude`, `longitude`). L'auteur décide de l'ajouter (saisie manuelle, recherche Nominatim, ou géolocalisation). Pastille façon TikTok sur le flux immersif ; tap → feuille carte + bouton « Y aller » (Google Maps / OSM). Un mood sans lieu n'affiche aucune pastille. 3 clips seed ont une adresse, 1 n'en a pas, pour illustrer le caractère facultatif. |
