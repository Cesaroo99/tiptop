...
export default function Page() {
  ...
  async function load() {
    const data = await api<{ items: Row[] }>("/admin/moods");
    setItems(data.items);
  }
  ...
  async function hide(id: string, next: boolean) {
    await api(`/admin/moods/${id}/hide`, { method: "POST", body: JSON.stringify({ hide: next }) });
    await load();
  }
      ...
      <div className="space-y-2">
        {items.map((m) => (
          <article key={m.id} className="rounded-card bg-surface p-4 shadow-card">
            <p className="text-sm">{m.body || m.kind}</p>
            <p className="mt-1 text-xs text-muted">
              @{m.author.username}
              {m.hidden ? ` · ${messages.admin.hidden}` : ""}
            </p>
            <button
              type="button"
              className="mt-3 rounded-pill bg-[var(--border)] px-3 py-2 text-sm"
              onClick={() => void hide(m.id, !m.hidden)}
            >
              {m.hidden ? messages.admin.unhide : messages.admin.hide}
            </button>
          </article>
        ))}
      </div>
