...
test.describe("Recherche in-app", () => {
  ...
  test("loupe du header, suggestions, apply et résultat réel", async ({ page }) => {
    ...
    await expect(results.getByRole("link", { name: /Erica Sinclair/ }).first()).toBeVisible();
    await expect(results.getByText(/Piscine party/).first()).toBeVisible();
    ...
    await expect(results.getByText("Piscine party - Odza, Yaoundé").first()).toBeVisible();
    await expect(results.getByText(/participants/).first()).toBeVisible();
    await expect(results.getByLabel("Coup de cœur").first()).toBeVisible();
  });
  ...
  test("loupe Events préremplit l’onglet sorties", async ({ page }) => {
    ...
    await expect(page.getByTestId("search-results").getByText(/Piscine party|Live session|Afterwork|Black/).first()).toBeVisible();
  });
